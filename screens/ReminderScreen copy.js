...
export default function ReminderScreen({ navigation }) {
  ...
  const setReminder = async () => {
    try {
      ...
      const userId = selectedUser  === 'myself' ? user?.uid : selectedUser ; // Caregiver sets a reminder for themselves or linked patient
      ...
      const reminderData = {
        ...
        status: 'active',
        daysOfWeek: selectedRepetition === 'weekly' || selectedRepetition === 'monthly'
      ...
      try {
        await axios.post('http://192.168.10.8/reminder', reminderData);
      ...
      if (selectedUser  !== 'myself') {
        try {
          ...
          console.error('Failed to send reminder to patient device:', error);
        }
      }
